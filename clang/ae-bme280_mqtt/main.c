...
extern int mqtt_message_send(char *);
...
int measure_temperature(int devfd)
{
    ...
    return( (int)((temp_data * 5 + 128) >> 8) / 10);
}

int measure_humidity(int devfd)
{
    ...
    return( ((int)(v_x1 >> 12)*10)/ 1024);
}

int measure_pressure(devfd)
{
...
int main()
{
    ...
    json_object_set(json_obj, "epoch_time", json_integer(time(NULL)) );
    json_object_set(json_obj, "temperature", json_integer(measure_temperature(devfd)) );
    json_object_set(json_obj, "humid", json_integer(measure_humidity(devfd)) );
    json_object_set(json_obj, "pressure", json_integer(measure_pressure(devfd)) );
    //printf("%s\n", json_dumps(json_obj, json_object_size(json_obj)));

    mqtt_message_send(json_dumps(json_obj, json_object_size(json_obj)));
    return(EXIT_SUCCESS);
}
